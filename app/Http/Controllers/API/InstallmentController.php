...
{
 public function index(Request $request){$q=Installment::with(['plan','booking.customer','booking.property']);foreach(['booking_id','installment_plan_id','status'] as $f)if($request->filled($f))$q->where($f,$request->$f);if($request->filled('from'))$q->whereDate('due_date','>=',$request->from);if($request->filled('to'))$q->whereDate('due_date','<=',$request->to);$perPage=min(max((int)$request->get('per_page',20),1),100);return response()->json($q->orderBy('due_date')->paginate($perPage));}
