...
export default {
  ...
  methods:{
    ...
    async save(addDocument){if(!this.$can('payments.create'))return;if(!this.form.customer_id||!this.form.booking_id||!this.form.amount){this.$root.$emit('show-error','Customer, booking and amount are required.');return}this.saving=true;try{const r=await api.post('/payments',this.form);const saved=r.data&&r.data.payment?r.data.payment:null;this.dialog=false;await this.load();this.$root.$emit('show-success',(r.data&&r.data.message)||'Payment recorded successfully.');if(addDocument&&saved){this.documentEntity=saved;this.documentsDialog=true}}catch(e){this.$root.$emit('show-error',(e.response&&e.response.data&&e.response.data.message)||'Unable to record payment.')}finally{this.saving=false}},
