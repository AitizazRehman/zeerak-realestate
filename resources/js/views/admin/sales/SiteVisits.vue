...
<div class="page">
  ...
  <v-dialog v-model="dialog" max-width="720" persistent>
    <v-card>
      ...
      <v-card-actions class="px-6 pb-5"><v-spacer/><v-btn text @click="dialog=false">Cancel</v-btn><v-btn v-if="editing ? $can('site_visits.edit') : $can('site_visits.create')" color="#165134" dark depressed :loading="saving" :disabled="!form.customer_id || !form.visit_date" @click="save">{{editing?'Update Visit':'Schedule Visit'}}</v-btn></v-card-actions>
    </v-card>
  </v-dialog>
...
export default{
 name:'SiteVisits',
 data(){return{loading:false,saving:false,dialog:false,deleteDialog:false,deleting:false,deleteItem:null,editing:null,items:[],customers:[],properties:[],loadingCustomers:false,loadingProperties:false,search:'',statusFilter:null,statusOptions:[{text:'Scheduled',value:'scheduled'},{text:'Completed',value:'completed'},{text:'Cancelled',value:'cancelled'},{text:'No Show',value:'no_show'}],headers:[{text:'Customer',value:'customer.name'},{text:'Property',value:'property.property_number'},{text:'Visit',value:'visit_at'},{text:'Status',value:'status'},{text:'Notes',value:'notes'},{text:'Actions',value:'actions',sortable:false}],form:this.blank()}},
 computed:{
 ...
 mounted(){this.load()},
 methods:{
  blank(){return{customer_id:null,property_id:null,visit_date:'',visit_time:'',status:'scheduled',feedback:'',notes:''}},
  toggleStatus(s){this.statusFilter=this.statusFilter===s?null:s},
  async loadLookups(){this.loadingCustomers=true;this.loadingProperties=true;try{const[c,p]=await Promise.all([api.get('/customers',{params:{per_page:100}}),api.get('/properties',{params:{per_page:100}})]);this.customers=c.data.data||[];this.properties=p.data.data||[]}catch(e){this.$root.$emit('show-error','Unable to load visit selections.')}finally{this.loadingCustomers=false;this.loadingProperties=false}},
  ...
  open(){this.editing=null;this.form=this.blank();this.form.visit_date=new Date().toISOString().slice(0,10);this.form.visit_time='10:00';this.dialog=true;this.loadLookups()},
  edit(item){this.editing=item;const d=item.visit_at?new Date(item.visit_at):new Date();this.form={customer_id:item.customer_id||null,property_id:item.property_id||null,visit_date:d.toISOString().slice(0,10),visit_time:d.toTimeString().slice(0,5),status:item.status||'scheduled',feedback:item.feedback||'',notes:item.notes||''};this.dialog=true;this.loadLookups()},
  async save(){this.saving=true;try{const visit_at=this.form.visit_date+' '+(this.form.visit_time||'00:00');const payload={customer_id:this.form.customer_id,property_id:this.form.property_id||null,visit_at,status:this.editing?this.form.status:'scheduled',feedback:this.editing?this.form.feedback||null:null,notes:this.form.notes};if(this.editing)await api.put('/site-visits/'+this.editing.id,payload);else await api.post('/site-visits',payload);this.dialog=false;await this.load();this.$root.$emit('show-success',this.editing?'Site visit updated successfully.':'Site visit scheduled successfully.');this.editing=null}catch(e){this.$root.$emit('show-error',(e.response&&e.response.data&&e.response.data.message)||'Unable to save site visit.')}finally{this.saving=false}},
