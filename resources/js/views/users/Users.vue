...
export default {
  ...
  data: () => ({
    ...
    headers: [{ text: 'Name', value: 'name' }, { text: 'Email', value: 'email' }, { text: 'Role', value: 'roles', sortable: false }, { text: 'Branch', value: 'branch.name', sortable: false }, { text: 'Created', value: 'created_at' }, { text: '', value: 'actions', sortable: false, align: 'right' }],
  ...
  methods: {
    ...
    openEdit (item) { this.editing = item; this.form = { name: item.name || '', email: item.email || '', password: '', role: item.roles && item.roles.length ? item.roles[0].name : null, branch_id: item.branch_id || null }; this.formError = ''; this.dialog = true },
    async save () {
      if (!this.$refs.form.validate()) return
      this.saving = true; this.formError = ''
      try {
        const payload = { name: this.form.name, email: this.form.email, role: this.form.role, branch_id: this.form.branch_id }
